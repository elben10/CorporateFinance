...
knitr::include_graphics("/Users/christofferelben/Library/Mobile Documents/com~apple~CloudDocs/Studier/cand.merc.fin/1st Semester/Corporate Finance/Notes/figures/placeholder.png")
```
...
*Prove that $E\left[\left(\tilde{r}- \bar{r}\right)^2\right] = E\left(\tilde{r}^2\right) - \tilde{r}^2$ using the following steps:* [@book, p.116]

a. *Show that $E\left[\left(\tilde{r} -\bar{r}\right)^2\right] = E\left(\tilde{r}^2-2\bar{r}\tilde{r}+\bar{r}\right)^2$.* [@book, p.116]

b. *Show that the expression in part $a$ is equal to $E\left(\tilde{r}^2\right)-2E\left(\bar{r}\tilde{r}\right)+\bar{r}^2$.* [@book, p.116]

c. *Show that the expression in part $b$ is equal to $E\left(\tilde{r}^2\right)-2\bar{r}^2+\bar{r}^2$.* [@book, p.116]
...
$$var\left(\tilde{R}_\rho\right) = x^2\sigma^2_1 + \left(1-x\right)^2\sigma^2_2+2x\left(1-x\right)\rho\sigma_1\sigma_2$$
...
```{r fig.align="center", echo=FALSE, out.width = "150px",}
knitr::include_graphics("/Users/christofferelben/Library/Mobile Documents/com~apple~CloudDocs/Studier/cand.merc.fin/1st Semester/Corporate Finance/Notes/figures/matrix.jpg")
```
...
$$var\left(\tilde{r}\right)=E\left[\left(\tilde{r}-\bar{r}\right)^2\right]$$
...
b. *Show that the alternative variance formula, $E\left(\tilde{r}^2\right)-E\left[\left(\tilde{r}-\bar{r}\right)^2\right]$, from exercise 4.1, yields the same results.* [@book, p.118]
...
*Assuming that the three economic scenarios are equally likely, compute the covariances and the correlation matrix for the four ABCO subsidiaries. Show that an alternative covariance formula, $cov\left(\tilde{r}_1,\tilde{r}_2\right) = E\left(\tilde{r}_1,\tilde{r}_2\right)-E\left(\tilde{r}_1\right)E\left(\tilde{r}_2\right)$, generates the same covariances.* [@book, p.118]
...
$$\tilde{r}_A=0.13+6\tilde{F}_1+4\tilde{F}_2+\tilde{\varepsilon}_A$$
$$\tilde{r}_B=0.15+2\tilde{F}_1+\tilde{F}_2+\tilde{\varepsilon}_B$$
$$\tilde{r}_C=0.07+5\tilde{F}_1-1\tilde{F}_2+\tilde{\varepsilon}_C$$
*If $var\left(\tilde{\varepsilon}_A\right)=0.01$ $var\left(\tilde{\varepsilon}_B\right)=0.4$ $\left(\tilde{\epsilon}_C\right)=0.02$, what are the variances of the returns of the three securities, as well as the covariances and correlations between them?* [@book, p.198]
...
$$\tilde{r}_{uni}=0.11+2\tilde{F}+\tilde{\varepsilon}_{uni}$$
$$\tilde{r}_{due}=0.17+5\tilde{F}+\tilde{\varepsilon}_{due}$$
...
$$r_1=0.06+2\tilde{F}_1+2\tilde{F}_2$$
$$r_2=0.05+3\tilde{F}_1+1\tilde{F}_2$$
$$r_3=0.04+3\tilde{F}_1+0\tilde{F}_2$$
...
$$r_4=0.08+1\tilde{F}_1+0\tilde{F}_2$$
...
$$Profit_{BP}=GBP \ 1 \ billion+GBP \ 10 \ million \ \tilde{F}_{OIL}+GBP \ 20 \ million \ \tilde{F}_{GBP/USD}+\tilde{\varepsilon}_{BP}$$
*Assume that each one-year oil futures contract purchased has the factor equation* [@book, p.752]
$$\tilde{C}_{OIL}=GBP \ 10,000 \ \tilde{F}_{OIL}$$
*Each one-year futures contract on the £/US\$ exchange rate has the factor equation* [@book, p.752]
$$\tilde{C}_{GBP/USD}=GBP \ 100,000 \ \tilde{F}_{GBP/USD}$$
...
$${Euro \ profits}_t = USD \ 10 \ mil.+ USD \ 8 \ mil. \times \left(USD/EUR \ 1-year \ forward \ exchange \ rate\right)_t+\tilde{\varepsilon}_t$$
